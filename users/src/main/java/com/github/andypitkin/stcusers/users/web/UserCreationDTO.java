...
/**
 * A DTO describing the data that needs to be provided when making a user creation request.
 * 
 */
public class UserCreationDTO {

    //ensure usernames can only be in the form of an email address so as to not get any malicious strings
    @NotBlank
    @Email
    private final String username;
    //allowing all characters, database queries are relying on JPA which makes use of paramaterised statements underneath
    //a theoretical frontend would need to sanitize these values for display to ensure XSS attacks don't take place
    @NotBlank
